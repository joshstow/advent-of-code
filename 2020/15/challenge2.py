# -*- coding: utf-8 -*-
"""
Created on Fri Jan 29 23:42:03 2021
...
while i < 30000000:
    ...
    if len(visited[prev]) == 1:
        visited[0].append(i)
        if len(visited[0]) == 3:    # Delete first element if length of list > 2
            del visited[0][0]
        prev = 0
        i += 1
        continue
    ...
    if diff not in visited:
        ...
        if len(visited[diff]) == 3: # Delete first element if length of list > 2
            del visited[diff][0]
    prev = diff
    i += 1
